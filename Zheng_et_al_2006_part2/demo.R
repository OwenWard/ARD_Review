...
fit <- stan(file = 'Zheng_et_al_2006_part2/NB_norecall.stan',
            data = data, 
            warmup = 1000,
            iter = 2000,
            cores = 4,
...
plot(alpha, 
     alpha_hat, 
     ylim = c(min(alpha_lower), max(alpha_upper)),
     xlab = expression(alpha),
     ylab = expression(hat(alpha)))
for (i in 1:I) lines(c(alpha[i], alpha[i]),
                     c(alpha_lower[i], alpha_upper[i]),
                     col = "grey")
points(alpha, alpha_hat, pch = 16)
abline(0, 1, col = "red")
plot(beta, 
     beta_hat,
     ylim = c(min(beta_lower), max(beta_upper)),
     xlab = expression(beta),
     ylab = expression(hat(beta)))
for (k in 1:K) lines(c(beta[k], beta[k]), 
                     c(beta_lower[k], beta_upper[k]),
                     col = "grey")
points(beta, beta_hat, pch = 16)
abline(0, 1, col = "red")
plot(omega, 
     omega_hat,
     ylim = c(min(omega_lower), max(omega_upper)),
     xlab = expression(omega),
     ylab = expression(hat(omega)))
for (k in 1:K) lines(c(omega[k], omega[k]),
                     c(omega_lower[k], omega_upper[k]),
                     col = "grey")
