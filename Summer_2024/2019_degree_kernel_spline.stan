// October 21st 2024
// this is a slightly simplified version of the original model 
// in the 2019 paper, assuming just name information
...
data {
  ...
  array[N, K] int Y; // responses to how many X do you know
  ...
  array[K] int<lower=1> g_k; // genders of names: 1=female 2=male (names have one each, but occs have two genders each)
  
  array[K] real<lower=0> mu_k; // age mean for each gender-group k
  array[K] real<lower=0> sigma_k; // age standard deviation for each gender-group k
  array[K] real<lower=0> sum_prob_k; // sum population proportion of each gender-group k
...
generated quantities {
  ...
  for (n in 1:N) {
    for (k in 1:K) {
      if(Y[n,k]>=0) {
        real mu_nk = rho[g_n[n], g_k[k]] * sum_prob_k[k]
                * norm_prod_int(age[n], mu_k[k],
                                lambda[g_n[n], g_k[k]][age[n] - 17],
                                sigma_k[k] ^ 2);
        mu_nk = mu_nk * d[n];
        y_sim[n,k] = neg_binomial_rng(omega[k] * mu_nk, omega[k]);
      }
      else{
        y_sim[n, k] = 0;
      } 
