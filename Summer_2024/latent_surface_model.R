### code taken from https://github.com/MengjiePan/BCMP/blob/master/latent_surface_model.R
### to fit the latent surface model to data in R
...
f.metro <- function(data, n.iter = 3000, m.iter = 3, n.thin = 10, ls.dim = 3, total.prop, z.pos.init, muk.fix) {
  ...
  dimnames(sims) <- list(NULL, NULL, c(
    paste("alpha", 1:data.n, sep = ""),
    paste("beta", 1:data.j, sep = ""),
    "a.mu", "a.s", "b.mu", "b.s", paste("etak", 1:data.j, sep = ""), "eta"
  ))
  dimnames(sims.latent) <- list(NULL, NULL, c(
    paste("respls", 1:data.n, sep = ""),
    paste("subpopls", 1:data.j, sep = "")
  ), NULL)
  sims.p <- array(NA, c(n.keep, m.iter, data.n + data.j + data.n + data.j + data.j + 1))
  dimnames(sims.p) <- list(NULL, NULL, c(
    paste("alpha", 1:data.n, sep = ""),
