...
degree_combined_mixing_code = '
  ...
  model {
    ...
    for(n in 1:N) {
      for(j in 1:J) {
        if(y[n,j]>=0) {
          mu_i <- omegaY[j] * d[n] * dot_product(M[ego[n]], sub_col(BetaY, 1, j, A));
          mu_i <- pow(mu_i, p);
          y[n,j] ~ neg_binomial(mu_i, omegaY[j]);
        }
      }
      for(k in 1:K) {
        if(z[n,k]>=0) {
          mu_i <- omegaZ[k] * d[n] * dot_product(M[ego[n]], sub_col(BetaZ, 1, k, A));
          mu_i <- pow(mu_i, p);
          z[n,k] ~ neg_binomial(mu_i, omegaZ[k]);
        }
      }
    }
  }
